...
graphical_summary = function(option_list)
{
  ...
  for(i in 1:length(array_features))
  {
    ...
    Annotation_file_subset_sel_HITS<-merge(Annotation_file_subset_sel_HITS,
                                           ATAC_CHIP_seq_HITS_subset.m,
                                           by=c("VAR_38","rs"))
    
    if(DEBUG ==1)
    {
      cat("Annotation_file_subset_sel_HITS_1\n")
      cat(str(Annotation_file_subset_sel_HITS))
      cat("\n")
    }
   ...
   ########################################### GRAPH ##########################################
    ...
    A<-round(summary(Annotation_file_subset_sel_HITS$value_Z_score[!is.na(Annotation_file_subset_sel_HITS$value_Z_score)]),2)
    ...
    graph_variable<-ggplot()+
      geom_sina(data=Annotation_file_subset_sel_HITS,
                 aes(x=BIN, 
                     y=value_Z_score), size=3)+
      geom_boxplot(data=Annotation_file_subset_sel_HITS,
                   aes(x=BIN, 
                       y=value_Z_score),
                   notch = TRUE,
                   notchwidth = 0.5)+
    ...
    graph_variable<-graph_variable+
      facet_grid(Annotation_file_subset_sel_HITS$cell_type ~ Annotation_file_subset_sel_HITS$HiChIP_loops, scales='free_x', space='free_x') +
    ...
    svgname<-paste(paste(features_sel,"Z_score", sep='_'),".svg",sep='')
    # svgname<-'test.svg'
    makesvg = TRUE
